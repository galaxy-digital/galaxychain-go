...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"rlv": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + rlv.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + rlv.blockNumber + " (" + new Date(1000 * rlv.getBlock(rlv.blockNumber).timestamp) + ")\n";
		try {
